const users = [
  ...
  // user의 id와 name, age를 가진 새로운 array 생성
  ...
  // user의 age가 27 이상인 사람의 id 추출
  const filteredIds = users
    .filter(user => user.age >= 27)
    .map(user => user._id);
  
  // id가 6271fd40e30856d15651c60c 인 사람의 name 추출
  const targetId = "6271fd40e30856d15651c60c";
  const targetUser = users.find(user => user._id === targetId);
  const targetUserName = targetUser ? targetUser.name : "사용자를 찾을 수 없음";
  
  // (더 나아가기) user의 평균 나이 구하기
  const totalAge = users.reduce((sum, user) => sum + user.age, 0);
  const averageAge = totalAge / users.length;
  
  // 결과 출력
  console.log("user의 id와 name, age를 가진 array:");
  console.log(userInfos);
  
  console.log("user의 age가 27 이상인 사람의 id:");
  console.log(filteredIds);
  
  console.log("id가 6271fd40e30856d15651c60c 인 사람의 name:");
  console.log(targetUserName);
  
  console.log("user의 평균 나이:");
